...
public final class StdRandom {
     ...
     * &alpha, lower bound parameter &lower and upper bound parameter &upper
     * 
     * This method is written by Ali Mortazavi. It is added to this class, because we 
     * need to have lower and upper bound for Pareto distribution for many applications
     * such as this simulator.
     * 
     * @param alpha shape parameter
     * @param lower  lower bound parameter
     * @param upper  upper bound parameter
     * @return a random real number from a Pareto distribution with shape
     * parameter <tt>alpha</tt> and lower bound parameter <tt>lower</tt> and
     * upper bound parameter <tt>upper</tt>
     * @throws IllegalArgumentException unless <tt>alpha > 0.0</tt>
     */
    public static double boundedPareto(double alpha, double lower, double upper) {
        if (!(alpha > 0.0)) {
            throw new IllegalArgumentException("Shape parameter alpha must be positive");
        }
        double a = (Math.pow(1 - uniform(), -1.0 / alpha) - 1.0) + lower;
        if (a < upper) {
            return a;
        }
